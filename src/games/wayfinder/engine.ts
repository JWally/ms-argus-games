...
const FUEL_FACTOR = 1.2; // 20 % buffer
const CITY_RADIUS = 9;
...
export function loadLeaderboard(n: number): LBEntry[] {
  try {
    return JSON.parse(localStorage.getItem(lbKey(n)) ?? '[]');
  } catch {
    return [];
  }
}
...
function saveScore(n: number, overshoot: number): { board: LBEntry[]; rank: number } {
  ...
  try {
    localStorage.setItem(lbKey(n), JSON.stringify(top));
  } catch {
    /* ignore */
  }
...
export function visitCity(state: GameState, cityId: number): GameState {
  ...
  if (route.length > 1) {
    routeLength += cityDist(state.cities[route[route.length - 2]], state.cities[cityId]);
  }
...
function placeCenters(k: number, w: number, h: number): { x: number; y: number }[] {
  ...
  while (centers.length < k && attempts < 3000) {
    ...
    if (!centers.some((c) => Math.hypot(c.x - x, c.y - y) < MIN_CENTER_SEP)) centers.push({ x, y });
  }
  return centers;
}
...
export function render(ctx: CanvasRenderingContext2D, state: GameState): void {
  ...
  if (animWaiting) {
    drawAnalysingLabel(ctx, state, false);
    return;
  }
  if (animRunning) {
    drawOptRouteAnimated(ctx, state);
    drawAnalysingLabel(ctx, state, true);
    return;
  }
...
function drawGrid(ctx: CanvasRenderingContext2D, w: number, h: number, _frame: number): void {
  ...
  for (let y = step; y < h; y += step) {
    ...
    ctx.fillStyle = GRID_TEXT;
    ctx.textAlign = 'left';
    ctx.fillText(String(row), 2, y - 2);
    ctx.textAlign = 'right';
    ctx.fillText(String(row), w - 2, y - 2);
    row++;
  }
}
...
function drawOptRoute(ctx: CanvasRenderingContext2D, state: GameState): void {
  ...
  ctx.strokeStyle = OPT_DIM;
  ctx.lineWidth = 2;
  ctx.setLineDash([8, 5]);
  ctx.lineJoin = 'round';
  ctx.shadowColor = OPT_COLOR;
  ctx.shadowBlur = 6;
  ctx.beginPath();
...
function drawOptRouteAnimated(ctx: CanvasRenderingContext2D, state: GameState): void {
  ...
  if (seg < route.length - 1 && t > 0) {
    const f = state.cities[route[seg]],
      to = state.cities[route[seg + 1]];
  ...
  if (seg < route.length - 1) {
    const f = state.cities[route[seg]],
      to = state.cities[route[Math.min(seg + 1, route.length - 1)]];
    ctx.beginPath();
...
function drawRoutingHud(ctx: CanvasRenderingContext2D, state: GameState): void {
  ...
  roundRect(ctx, barX, gaugeY + 6, barW, barH, 3);
  ctx.fill();
  ...
  ctx.shadowColor = fuelColor;
  ctx.shadowBlur = 4;
  roundRect(ctx, barX, gaugeY + 6, barW * pct, barH, 3);
  ctx.fill();
  ctx.shadowBlur = 0;
...
const BRIEF_STATIC = [
  {
    label: 'SITUATION:',
    body: 'YOUR TROOPS ARE RUNNING LOW ON SUPPLIES AND YOU ARE RUNNING LOW ON FUEL.',
  },
  {
    label: 'YOUR MISSION:',
    body: "PLOT THE SHORTEST ROUTE TO DELIVER SUPPLIES TO ALL YOUR TROOPS, WHILE CONSERVING AS MUCH FUEL AS POSSIBLE. WE'RE COUNTING ON YOU! GOOD LUCK!",
  },
];
...
function drawReadyOverlay(ctx: CanvasRenderingContext2D, state: GameState): void {
  ...
  if (revealed < totalChars && Math.floor(state.frameCount / 18) % 2 === 0) {
    ...
    ctx.shadowColor = RED_GLOW;
    ctx.shadowBlur = 8;
    ctx.fillRect(cursorX, cursorY - 13, 9, 15);
    ctx.shadowBlur = 0;
  }
  ...
  const bw = 160,
    ...
    by = height - 60;
  ctx.shadowColor = RED_GLOW;
  ctx.shadowBlur = revealed >= totalChars ? 16 : 4;
  ...
  ctx.textAlign = 'center';
  ctx.shadowColor = RED_GLOW;
  ctx.shadowBlur = revealed >= totalChars ? 12 : 0;
  ctx.fillText("LET'S GO", cx, by + bh / 2 + 6);
  ctx.shadowBlur = 0;
}
...
function drawDoneOverlay(ctx: CanvasRenderingContext2D, state: GameState): void {
  ...
  ctx.shadowBlur = 6;
  ctx.fillText(`${playerLen} km`, width / 2, y);
  ctx.shadowBlur = 0;
  ...
  ctx.fillText(isOptimal ? 'PERFECT EFFICIENCY' : `+${overshoot}% OVER ${optLabel}`, width / 2, y);
  ...
  if (lb.length > 0) {
    ...
    for (let i = 0; i < lbRows; i++) {
      ...
      ctx.textAlign = 'left';
      ctx.fillText(
        `#${i + 1}  ${entry.overshoot === 0 ? 'OPTIMAL' : `+${entry.overshoot}%`}`,
        panelX + pad,
        y
      );
      ctx.textAlign = 'right';
  ...
  ctx.shadowBlur = 6;
  ctx.fillStyle = '#3a0a0a';
  roundRect(ctx, btnX, btn1Y, btnW, btnH, 4);
  ctx.fill();
  ...
  roundRect(ctx, btnX, btn2Y, btnW, btnH, 4);
  ctx.fill();
...
function drawFailedOverlay(ctx: CanvasRenderingContext2D, state: GameState): void {
  const { width, height } = state;
  const panelW = Math.min(width - 32, 280),
    panelH = 130;
  const panelX = (width - panelW) / 2,
    panelY = height / 2 - panelH / 2 - 20;
  ...
  ctx.shadowBlur = 4;
  ctx.beginPath();
  ctx.moveTo(panelX + pad, panelY + 28);
  ctx.lineTo(panelX + panelW - pad, panelY + 28);
  ctx.stroke();
  ctx.shadowBlur = 0;
  ...
  ctx.shadowBlur = 6;
  ctx.fillText(`${used} km`, width / 2, panelY + 80);
  ctx.shadowBlur = 0;
...
export function doneButtonAtPoint(
  state: GameState,
  px: number,
  py: number
): 'retry' | 'new' | null {
