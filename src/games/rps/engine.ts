...
const tGet = (t: FreqTable, k: string): [number, number, number] =>
  t[k] ?? [0, 0, 0];
...
function predictHuman(rounds: Round[]): Choice {
  ...
  const hitRates = mem.trials.map((t, i) =>
    t < MIN_TRIALS ? 1 / 3 : mem.hits[i] / t,
  );
  ...
  if (bestIdx === -1) {
    ...
    if (total >= 2) {
      ...
      for (let i = 0; i < 3; i++) { r -= counts[i]; if (r < 0) return CHOICES[i]; }
    }
    return randomChoice();
  }

  const prediction   = preds[bestIdx]!;
  const trustFraction = Math.min(
    1,
    (bestRate - NASH_FLOOR) / (TRUST_CEIL - NASH_FLOOR),
  );
...
export function finalizeRound(
  ...
  return {
    newState: {
      ...state, rounds, humanScore, aiScore, seriesOver,
...
export function saveResult(winner: 'human' | 'ai' | 'tie'): void {
  ...
  try { localStorage.setItem('rps-record', JSON.stringify(r)); } catch { /* empty */ }
}
